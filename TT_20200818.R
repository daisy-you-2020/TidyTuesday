...
plants %>% group_by(red_list_category) %>% summarise(plt_n = n_distinct(binomial_name))

#extinct plant numbers by country and continent
ext_plt_by_c = plants %>% group_by(continent, country) %>% summarise(plt_n = n_distinct(binomial_name)) %>% ungroup()


ext_plt_by_c_map = joinCountryData2Map(ext_plt_by_c, joinCode="NAME", nameJoinColumn="country", verbose=TRUE)
...
ext_plt_by_c = ext_plt_by_c %>% mutate(country = ifelse(country == 'Cabo Verde', 'Cape Verde', ifelse(country == "Côte d'Ivoire", "Ivory Coast", country)))

#plot
theme_set(theme_classic() + theme(legend.position = "top", plot.title.position = 'plot', plot.title = element_text(hjust = 0.5), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)), axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0))))

mapCountryData(ext_plt_by_c_map, nameColumnToPlot="plt_n", mapTitle="Extinct Plants by Countries", colourPalette = 'heat', catMethod='fixedWidth', numCats=100)
